...
public class WorldPacket : ByteBuffer
{
    ...
    public Opcode GetUniversalOpcode(bool isModern)
    {
        if (isModern)
        {
            ...
            if (IsWotLK343LearnPreviewTalents(GetOpcode(), ModernVersion.GetCurrentOpcode(Opcode.CMSG_LEARN_TALENT)))
    ...
    internal static bool IsWotLK343LearnPreviewTalents(uint opcode, uint learnTalentOpcode) =>
        learnTalentOpcode == 0x3552 && opcode == 0x3553;
