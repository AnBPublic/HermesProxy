...
public class ObjectUpdateBuilder
{
    ...
    private void BuildMovementUpdate(WorldPacket data)
    {
        ...
        if (Has(CreateObjectBits.ActivePlayer))
        {
            ...
            // action+type), unlike the standalone SMSG which uses int64. The
            // 3.4.3 descriptor reserves 180 buttons.
            for (int j = 0; j < 180; j++)
                data.WriteInt32(0);
        }
    }
    ...
    private void WriteCreatePlayerData(WorldPacket data)
    {
        ...
        data.WritePackedGuid128(IsOwner
            ? _updateData.GlobalSession.GetBnetAccountGuidForPlayer(_updateData.Guid)
            : WowGuid128.Empty);
        data.WriteUInt32(0u);
        for (int l = 0; l < 19; l++)
            data.WriteUInt32(0u);
